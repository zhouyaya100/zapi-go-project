<template>
  <div>
    <div class="page-header"><h2 class="page-title">{{ t('log.title') }}</h2></div>
    <div class="filter-bar">
      <el-input v-model="filters.model" placeholder="Model" clearable style="width:150px" @clear="load" @keyup.enter="load" />
      <el-input v-model="filters.channel_id" placeholder="Channel ID" clearable style="width:120px" @clear="load" @keyup.enter="load" />
...
const filters = ref<{ model: string; channel_id: string; username: string; success: boolean | string; dateRange: Date[] | null }>({ model: '', channel_id: '', username: '', success: '', dateRange: null })
...
async function load() {
  ...
  try {
    ...
    const d = await logApi.operatorLogs(params)
...
function exportCsv() { exportApi.operatorCsv(getExportParams()) }
function exportXlsx() { exportApi.operatorXlsx(getExportParams()) }
</script>
