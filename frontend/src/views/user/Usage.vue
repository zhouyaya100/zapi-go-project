<template>
  <div>
    <div class="page-header">
      <h2 class="page-title">{{ t('nav.myUsage') }}</h2>
      <div style="display:flex;gap:8px">
        ...
        <el-select v-model="groupBy" style="width:120px" @change="load">
          <el-option label="按天" value="day" /><el-option label="按模型" value="model" />
        </el-select>
...
async function load() {
  ...
  try {
    ...
    usage.value = await statsApi.myUsage(params); total.value = usage.value?.total || 0
    await nextTick()
    setTimeout(() => {
...
function exportCsv() {
  ...
  if (modelFilter.value) params.model = modelFilter.value
  if (dateRange.value?.length === 2) { params.date_from = dateRange.value![0].toISOString().slice(0,10); params.date_to = dateRange.value![1].toISOString().slice(0,10) }
  exportApi.myCsv(params)
}
function exportXlsx() {
  const params: any = { group_by: groupBy.value }
  if (modelFilter.value) params.model = modelFilter.value
  if (dateRange.value?.length === 2) { params.date_from = dateRange.value![0].toISOString().slice(0,10); params.date_to = dateRange.value![1].toISOString().slice(0,10) }
  exportApi.myXlsx(params)
}
</script>
